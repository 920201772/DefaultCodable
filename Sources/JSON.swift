...
public extension JSON {
    ...
    static func decode(json data: Data, options: CodableOptions = DefaultCodableOptions, userInfo: [AnyHashable: Any] = [:]) throws -> Self {
    ...
    static func decode(json text: String, options: CodableOptions = DefaultCodableOptions, userInfo: [AnyHashable: Any] = [:]) throws -> Self {
        guard let data = text.data(using: .utf8) else {
            throw DecodingError.dataCorrupted(.init(codingPath: [], debugDescription: "Encoding utf-8 fail."))
        }
        
        return try decode(json: data, options: options, userInfo: userInfo)
    }
    
    static func decode(json url: URL, options: CodableOptions = DefaultCodableOptions, userInfo: [AnyHashable: Any] = [:]) throws -> Self {
        let data = try Data(contentsOf: url)
        return try decode(json: data, options: options, userInfo: userInfo)
    }
    
    static func decode(jsonPath: String, options: CodableOptions = DefaultCodableOptions, userInfo: [AnyHashable: Any] = [:]) throws -> Self {
        try decode(json: .init(fileURLWithPath: jsonPath), options: options, userInfo: userInfo)
    }
